...
class PullRequestReviewTest < ActionDispatch::IntegrationTest
  test "adds an inline comment on the overall pull request diff" do
    with_sample_repository do |fixture|
      ...
      post repository_pull_comments_path(repository, pull_request), params: {
        inline_comment: {
  ...
  test "shows a commits index page and steps through individual commits" do
    with_sample_repository do |fixture|
      ...
      assert_select "form[action='#{repository_pull_comments_path(repository, pull_request)}']"
  ...
  test "adds an inline comment to a commit diff line" do
    with_sample_repository do |fixture|
      ...
      post repository_pull_comments_path(repository, pull_request), params: {
        ...
        redirect_to: repository_pull_commit_path(repository, pull_request, fixture.feature_commits[:refine_widget])
      }

      assert_redirected_to repository_pull_commit_path(repository, pull_request, fixture.feature_commits[:refine_widget])
      follow_redirect!
